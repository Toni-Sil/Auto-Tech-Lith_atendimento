...
async def setup_mfa(
    ...
    uri = mfa_service.get_provisioning_uri(secret=secret, username=current_user.username)
...
async def verify_mfa_setup(
    ...
    if not is_valid:
        await log_security_event(db, "mfa_setup_failed", username=current_user.username, ip_address=request.client.host)
        raise HTTPException(status_code=400, detail="Invalid MFA code. Try again.")
        
    current_user.mfa_enabled = True
    await log_security_event(db, "mfa_enabled", username=current_user.username, ip_address=request.client.host)
    await db.commit()
...
async def disable_mfa(
    ...
    is_valid = mfa_service.verify_totp(current_user.mfa_secret, data.code)
    if not is_valid:
        await log_security_event(db, "mfa_disable_failed", username=current_user.username, ip_address=request.client.host)
        raise HTTPException(status_code=400, detail="Invalid MFA code.")
        
    current_user.mfa_enabled = False
    current_user.mfa_secret = None
    
    await log_security_event(db, "mfa_disabled", username=current_user.username, ip_address=request.client.host)
    await db.commit()
