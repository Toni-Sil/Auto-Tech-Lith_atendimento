...
async def export_customers_csv(
    ...
    return PlainTextResponse(
        content=csv_data, 
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=customers_{current_user.tenant_id}.csv"}
    )
...
async def export_tickets_csv(
    ...
    return PlainTextResponse(
        content=csv_data, 
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=tickets_{current_user.tenant_id}.csv"}
    )
