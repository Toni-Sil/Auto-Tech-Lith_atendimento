...
from src.schemas import \
    VerifyEmailRequest  # assuming I added this earlier to schemas
...
async def resolve_tenant(
    request: Request, host: str = None, db: AsyncSession = Depends(get_db)
):
    """
...
async def register_tenant(
    ...
    existing_tenant = await db.scalar(
        select(Tenant).where(Tenant.subdomain == reg_data.subdomain)
    )
    if existing_tenant:
    ...
    try:
        new_tenant = Tenant(name=reg_data.name, subdomain=reg_data.subdomain)
        db.add(new_tenant)
        ...
        logger.info(
            f"📧 Verification link for {new_admin.email}: /api/v1/tenant/verify-email?token={verify_token}"
        )
...
async def verify_tenant_email(
    ...
    try:
        payload = jwt.decode(
            verification.token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM]
        )
    ...
    admin = await db.scalar(
        select(AdminUser).where(AdminUser.username == token_username)
    )
    if not admin:
...
async def verify_tenant_phone(
    request: Request, data: VerifyPhoneRequest, db: AsyncSession = Depends(get_db)
):
    await rate_limit_check(request, limit=10, window_seconds=3600)
    ...
    if not admin or not admin.phone_otp:
        raise HTTPException(
            status_code=404,
            detail="Solicitação de OTP não encontrada para este telefone.",
        )

    if not verify_password(data.otp, admin.phone_otp):
        await log_security_event(
            db,
            "registration_phone_verify_failed",
            username=admin.username,
            tenant_id=admin.tenant_id,
            ip_address=request.client.host,
        )
