...
from src.schemas.automation import AutomationRuleCreate, AutomationRuleUpdate, AutomationRuleResponse
...
async def create_rule(
    ...
    current_user: Annotated[AdminUser, Depends(RequirePermissions(["automations:write"]))],
...
async def list_rules(
    current_user: Annotated[AdminUser, Depends(RequirePermissions(["automations:read"]))],
    db: AsyncSession = Depends(get_db)
):
...
async def update_rule(
    ...
    rule = await automation_service.update_rule(db, current_user.tenant_id, rule_id, data)
    if not rule:
