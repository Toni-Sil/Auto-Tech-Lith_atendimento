...
async def get_current_user(
    ...
    try:
        ...
        if jti:
            session_check = await db.scalar(
                select(UserSession).where(UserSession.session_token_jti == jti)
            )
...
class RequirePermissions:
    ...
    def __call__(self, current_user: AdminUser = Depends(get_current_user)):
        if not current_user.custom_role:
            raise HTTPException(
                status_code=403,
                detail="Acesso negado: Usuário sem perfil de acesso definido.",
            )
        ...
        for perm in self.required_permissions:
            if perm not in user_perms:
                raise HTTPException(
                    status_code=403, detail=f"Acesso negado: Falta permissão '{perm}'"
                )
...
async def login_for_access_token(
    ...
    user = await db.scalar(
        select(AdminUser).where(
            func.lower(AdminUser.username) == func.lower(form_data.username)
        )
    )
    ...
    if user.tenant_id:
        ...
        if tenant and tenant.status == "pending":
            ...
            raise HTTPException(
                ...
                detail="Conta pendente de aprovação (Tenant em análise).",
            )
        ...
        if user.role == "owner":
            if not user.email_verified:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Verificação de E-mail pendente. Confirme seu e-mail para continuar.",
                )
            if not user.phone_verified:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Verificação de Telefone (OTP) pendente. Confirme seu celular para continuar.",
                )
    ...
    is_valid_hash = user.password_hash and verify_password(
    ...
    if not (is_valid_hash or is_valid_code):
        logger.info(
            f"❌ Login failed for {user.username}: Hash Valid={is_valid_hash}, Code Valid={is_valid_code}"
        )
    ...
    if user.mfa_enabled:
        ...
        if not mfa_token:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="MFA_REQUIRED"
            )
...
async def request_password_recovery(
    ...
    user = await db.scalar(
        select(AdminUser).where(
            or_(
                ...
                == func.lower(recovery_data.email_or_username),
                (
                    func.lower(AdminUser.email)
                    == func.lower(recovery_data.email_or_username)
                    if recovery_data.email_or_username
                    else False
                ),
            )
        )
    ...
    if user.email:
        ...
        logger.warning(
            f"Usuário {user.username} solicitou recuperação mas não tem email cadastrado."
        )
...
async def reset_password(
    ...
    user = await db.scalar(
        select(AdminUser).where(AdminUser.username == reset_data.username)
    )
    ...
    if datetime.now(timezone.utc) > user.recovery_token_expires_at.replace(
        tzinfo=timezone.utc
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def read_users_me(current_user: Annotated[AdminUser, Depends(get_current_user)]):
    return {
        ...
        "permissions": (
            current_user.custom_role.permissions if current_user.custom_role else []
        ),
...
from src.schemas import (AdminUserProfileUpdate, PasswordChangeRequest,
                         VerifyEmailRequest)
...
async def update_user_me(
    ...
    if profile_data.email is not None and profile_data.email != current_user.email:
        ...
        logger.info(
            f"📧 [DUMMY EMAIL] Enviado link de verificação para {profile_data.email}"
        )
        logger.info(
            f"🔗 Link de confirmação de email: /verify-email?token={verify_token}"
        )
...
async def change_password(
    ...
    is_valid_hash = current_user.password_hash and verify_password(
        password_data.current_password, current_user.password_hash
    )
    is_valid_code = (
        current_user.access_code
        and password_data.current_password == current_user.access_code
    )
    ...
    if current_user.mfa_enabled:
        ...
        if not mfa_service.verify_totp(current_user.mfa_secret, mfa_token):
            await log_security_event(
                db,
                "password_change_failed_invalid_mfa",
                username=current_user.username,
                tenant_id=current_user.tenant_id,
                operator_id=current_user.id,
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="MFA inválido."
            )
