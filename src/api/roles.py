...
from src.schemas import RoleCreate, RoleResponse, RoleUpdate
...
async def create_role(
    ...
    role = await role_service.create_role(
        data, tenant_id=current_user.tenant_id, operator=current_user
    )
    ...
    if current_user.mfa_enabled:
        ...
        async with async_session() as session:
            is_valid = await mfa_service.verify_totp(
                current_user.id, mfa_token, session
            )
        if not is_valid:
            raise HTTPException(status_code=403, detail="Invalid MFA token")

    role = await role_service.update_role(
        role_id, data, tenant_id=current_user.tenant_id, operator=current_user
    )
    if not role:
        raise HTTPException(
            status_code=404, detail="Role not found or cannot be modified."
        )
    return RoleResponse.model_validate(role)
...
async def delete_role(
    ...
    success = await role_service.delete_role(
        role_id, tenant_id=current_user.tenant_id, operator=current_user
    )
    if not success:
        raise HTTPException(
            status_code=400, detail="Role not found or is a system role."
        )
