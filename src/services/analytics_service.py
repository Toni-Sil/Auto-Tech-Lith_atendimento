...
class AnalyticsService:
    async def get_overview_stats(self):
        async with async_session() as db:
            ...
            channel_stmt = select(Customer.source, func.count(Customer.id)).group_by(
                Customer.source
            )
    ...
    async def get_business_impact(self):
        async with async_session() as db:
            ...
            funnel_stmt = select(Customer.status, func.count(Customer.id)).group_by(
                Customer.status
            )
            ...
            leads_stmt = select(func.count(Customer.id)).where(
