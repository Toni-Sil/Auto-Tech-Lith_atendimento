...
class AutomationService:
    async def create_rule(
        self, session: AsyncSession, tenant_id: int, data: AutomationRuleCreate
    ) -> AutomationRule:
        rule = AutomationRule(
            tenant_id=tenant_id,
            name=data.name,
    ...
    async def _execute_action(
        self, session: AsyncSession, rule: AutomationRule, context: Dict[str, Any]
    ):
        ...
        try:
            if rule.action_type == "send_email":
                ...
                recipient_id = rule.action_payload.get("recipient_id") or context.get(
                    "owner_id"
                )
                if recipient_id:
                    msg = NotificationCreate(
                        channel="email",
                        title=rule.action_payload.get(
                    ...
                    await notification_service.create_notification(
                        session, rule.tenant_id, msg
                    )

            elif rule.action_type == "webhook":
                # Would do a HTTP POST to rule.action_payload["url"]
                logger.info(
                    f"Automation Webhook Fire: {rule.action_payload.get('url')} with {context}"
                )
