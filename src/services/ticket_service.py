...
class TicketService:
    ...
    async def get_ticket(
        self, ticket_id: int, tenant_id: Optional[int] = None
    ...
    async def update_ticket_status(
        ...
        async with async_session() as session:
            try:
                ...
                logger.info(
                    f"Ticket #{ticket_id} status: {old_status.value} → {status.value}"
                )
                return True
    ...
    async def list_customer_tickets(
        self, customer_id: int, status: Optional[TicketStatus] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _detect_category(self, subject: str, description: str) -> TicketCategory:
        ...
        if any(
            kw in text
            for kw in ["pagamento", "cobrança", "billing", "invoice", "fatura"]
        ):
            return TicketCategory.BILLING
        if any(kw in text for kw in ["integração", "api", "webhook", "connect"]):
            return TicketCategory.INTEGRATION
        if any(
            kw in text for kw in ["feature", "funcionalidade", "recurso", "sugestão"]
        ):
    ...
    def _ticket_to_dict(self, ticket: Any) -> Dict[str, Any]:
        ...
        return {
            ...
            "resolved_at": (
                ...
                if hasattr(ticket, "resolved_at") and ticket.resolved_at
                else None
            ),
        }
