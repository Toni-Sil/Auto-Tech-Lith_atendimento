...
class QuotaService:
    ...
    async def increment_message_counter(self, tenant_id: int, db: AsyncSession) -> None:
        ...
        pct_monthly = quota.current_messages_monthly / max(
            quota.max_messages_monthly, 1
        )
    ...
    async def decrement_whatsapp_instances(
        self, tenant_id: int, db: AsyncSession
    ) -> None:
        quota = await self.get_or_create(tenant_id, db)
