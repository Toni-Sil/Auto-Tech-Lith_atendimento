...
class ApiKeyService:
    ...
    async def create_api_key(self, session: AsyncSession, tenant_id: int, data: ApiKeyCreate) -> tuple[ApiKey, str]:
        """
        ...
        existing = await session.scalar(select(ApiKey).where(ApiKey.key_hash == hashed_key))
        if existing:
    ...
    async def verify_api_key(self, session: AsyncSession, plain_key: str) -> Optional[ApiKey]:
    ...
    async def delete_api_key(self, session: AsyncSession, tenant_id: int, api_key_id: int) -> bool:
        """Deletes an API Key."""
        api_key = await session.scalar(select(ApiKey).where(ApiKey.id == api_key_id, ApiKey.tenant_id == tenant_id))
        if api_key:
