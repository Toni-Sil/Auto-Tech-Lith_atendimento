...
class AuditLog(Base):
    ...
    tenant_id = Column(Integer, ForeignKey("tenants.id"), nullable=True, index=True)  # NEW
    operator_id = Column(Integer, ForeignKey("admin_users.id"), nullable=True, index=True)
    event_type = Column(String, index=True) # e.g., "login_success", "login_failure", "account_lockout"
    ...
    token_count = Column(Integer, nullable=True)  # NEW — LLM tokens for AI-related audit events
