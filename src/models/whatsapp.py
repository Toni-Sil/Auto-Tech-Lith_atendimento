...
class EvolutionInstance(Base):
    ...
    )  # Name created on Evolution API
    display_name = Column(String, nullable=True)  # E.g. "Suporte Master"
    instance_token = Column(
        String, nullable=True
    )  # Custom token/apikey for this instance
    evolution_api_url = Column(
        String, nullable=True
    )  # Evolution API server URL override
    ...
    )  # pending, connected, disconnected, expired
