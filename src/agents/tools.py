...
async def update_customer_info(
    ...
    async with async_session() as session:
        ...
        if demand:
            customer.initial_demand = demand
...
async def schedule_meeting(
    customer_id: int, meeting_type: str, date_str: str, time_str: str, notes: str = ""
) -> str:
            ...
            if customer.tenant_id is None:
                try:
                    ...
                    if lead:
                        ...
                        logger.info(
                            f"Sync: Lead {lead.id} status updated to {lead.status}"
                        )
                except Exception as e:
...
async def check_availability(
    tenant_id: Optional[int], date_str: Optional[str] = None
) -> str:
    """
    ...
    async with async_session() as session:
        for d in check_days:
            ...
            if not meetings:
                ...
                report.append(
                    f"📅 {day_str}: Ocupado em {', '.join(busy_slots)}. Demais horários livres."
                )
...
async def create_support_ticket(
    customer_id: int, subject: str, description: str, category: Optional[str] = None
) -> str:
    """
    ...
    """
    try:
        from src.services.ticket_service import TicketCategory, ticket_service
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "demand": {
                        "type": "string",
                        "description": "Relatório detalhado do serviço ou problema relatado (Obrigatório)",
                    },
                },
                "required": [],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "date_str": {
                        "type": "string",
                        "description": "Data específica para verificar (YYYY-MM-DD). Opcional.",
                    }
                },
                "required": [],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Termo de busca (nome do produto/serviço)",
                    },
                    "category": {
                        "type": "string",
