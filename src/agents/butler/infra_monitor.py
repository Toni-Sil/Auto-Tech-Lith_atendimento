...
def _check_docker_containers() -> List[ServiceHealth]:
    ...
    if not shutil.which("docker"):
        ...
        results.append(
            ServiceHealth(
                "docker",
                "ok",
                detail="Docker not installed/required in this environment",
            )
        )
        return results
    ...
    try:
        ...
        if not results:
            results.append(
                ServiceHealth("docker", "ok", detail="No containers running")
            )
    except FileNotFoundError:
        results.append(
            ServiceHealth("docker", "degraded", detail="docker binary not found")
        )
    except Exception as e:
...
async def run_infra_check(
    database_url: str, api_base: str = "http://localhost:8000"
) -> InfraStatus:
