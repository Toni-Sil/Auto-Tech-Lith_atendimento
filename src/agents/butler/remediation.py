    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if keys:
                ...
                logger.info(
                    f"Cache cleared: {deleted} keys deleted (pattern: {cache_pattern})"
                )
...
PLAYBOOKS = {
    "redis_down": [
        (
            RestartServiceAction("restart_redis", "Restart Redis service"),
            {"service_name": "redis"},
        ),
        (
            ClearCacheAction("verify_cache", "Verify cache accessibility"),
            {"pattern": "test:*"},
        ),
    ],
    "database_connection_lost": [
        ...
        (
            RestartServiceAction("restart_postgres", "Restart PostgreSQL if needed"),
            {"service_name": "postgres"},
        ),
    ],
    "evolution_disconnected": [
        (
            RestartServiceAction("restart_evolution", "Restart Evolution API"),
    ...
    "high_memory_usage": [
        (
            ClearCacheAction("clear_old_cache", "Clear old cache entries"),
            {"pattern": "ai_response:*"},
        ),
    ],
}
...
async def execute_playbook(
    playbook_name: str, context: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    for action, action_context in playbook:
        ...
        try:
            ...
            if result.get("status") == "failed":
                logger.warning(
                    f"Playbook {playbook_name} stopped at {action.name}: {result.get('error')}"
                )
                break
    ...
    return {
        ...
        "actions": results,
        "success": all(
            r["result"].get("status") in ["success", "simulated"] for r in results
        ),
    }
