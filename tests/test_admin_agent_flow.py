...
def test_admin_agent_denied_action():
    async def _test():
        with patch("src.agents.admin_agent.llm_service") as mock_llm_service, patch(
            "src.agents.admin_agent.async_session"
        ) as mock_db:
            ...
            with patch.object(
                agent, "identify_user", new_callable=AsyncMock
            ) as mock_identify:
                ...
                # Side effect
                mock_llm_service.get_chat_response.side_effect = [
                    llm_response,
                    MagicMock(content="Ação negada."),
                ]

                # 3. Exec
                response = await agent.process_message(
                    "Apague o cliente 123", context={"user_id": 1}
                )
    ...
    async def _test():
        with patch("src.agents.admin_agent.llm_service") as mock_llm_service, patch(
            ...
            with patch.object(
                ...
                mock_llm_service.get_chat_response.side_effect = [
                    llm_response,
                    MagicMock(content="Preciso de confirmação."),
                ]
    ...
    async def _test():
        with patch("src.agents.admin_agent.llm_service") as mock_llm_service, patch(
            ...
            with patch.object(
                ...
                response = await agent.process_message(
                    "Sim, confirmar apagar cliente 123", context={"user_id": 1}
                )
