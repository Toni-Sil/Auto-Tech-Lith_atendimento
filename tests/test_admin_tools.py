...
async def main():
    ...
    # Create
    fake_data = '{"name": "Teste Bot", "phone": "5511999990000", "status": "em_processo"}'
    ...
    try:
        ...
        res_update = await admin_agent.manage_customers(action="update", customer_id=cust_id, data=update_data)
        print(f"UPDATE: {res_update}")
        
        # 2. Manage Tickets
        print("\n--- Testing manage_tickets ---")
        res_ticket = await admin_agent.manage_tickets(action="create", customer_id=cust_id, subject="Ticket de Teste")
        print(f"CREATE TICKET: {res_ticket}")
        
        res_list_tickets = await admin_agent.manage_tickets(action="list", customer_id=cust_id)
        print(f"LIST TICKETS: {res_list_tickets}")
        
        # 3. Manage Meetings
        print("\n--- Testing manage_meetings ---")
        res_meeting = await admin_agent.manage_meetings(action="schedule", customer_id=cust_id, date_str="2026-12-31", time_str="15:00")
        ...
        res_delete = await admin_agent.manage_customers(action="delete", customer_id=cust_id)
